...
def save_model(model, save_path, name, dataset, iter_cnt):
    save_name = os.path.join(save_path, name + '_' + dataset + '_' + str(iter_cnt) + '.pth')
...
if __name__ == '__main__':
    ...
    if opt.metric == 'add_margin':
        ...
        metric_fc = metrics.ArcMarginProduct(512, opt.num_classes, s=30, m=0.5, 
                                                easy_margin=opt.easy_margin)
    elif opt.metric == 'sphere':
        metric_fc = metrics.SphereProduct(512, opt.num_classes, m=4)
    else:
        metric_fc = metrics.MyLinear(512, opt.num_classes)
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 
                                                step_size=opt.lr_step, gamma=0.1)
    ...
    for i in range(opt.max_epoch):
        ...
        if i % opt.save_interval == 0 or i == opt.max_epoch:
            save_model(model, opt.checkpoints_path, opt.backbone, opt.dataset, i)

        # testing model
        if i % opt.test_interval == 0 or i == opt.max_epoch:
            model.eval()
            acc = test(model, img_paths, identity_list, opt.test_pair_list, opt.test_batch_size)
