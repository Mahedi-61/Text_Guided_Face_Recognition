...
from utils.utils import mkdir_p,get_rank,merge_args_yaml,get_time_stamp,save_args
from utils.utils import load_model_opt, save_models, load_npz, params_count
from utils.perpare import prepare_dataloaders, prepare_models
from utils.datasets import get_fix_data 
from utils.modules import sample_one_batch as sample, test as test, train as train
...
def main(args):
    ...
    args.model_save_file = osp.join(ROOT_PATH, 'checkpoints', str(args.CONFIG_NAME), stamp)
    ...
    image_encoder, text_encoder, netG = prepare_models(args)
    ...
    if args.resume_epoch!=1:
        ...
        netG, netD, netC, optimizerG, optimizerD = load_model_opt(netG, 
                                netD, netC, optimizerG, optimizerD, path, args.multi_gpus)
    # print args
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
    ...
    for epoch in range(strat_epoch, args.max_epoch, 1):
        ...
        train(train_dl, netG, text_encoder, optimizerG, args)
        
        # save
        if epoch%save_interval==0:
            save_models(netG, optimizerG, epoch, args.multi_gpus, args.model_save_file)
